...
export async function execute(transactions?: Array<{ to: string; value: string; data: string }>): Promise<string> {
  ...
  try {
    ...
    // Ensure we have proper gas prices (at least 30 gwei for maxPriorityFeePerGas)
    const minPriorityFeePerGas = 30000000000n; // 30 gwei
    const minMaxFeePerGas = 30000000000n; // 30 gwei
    ...
    // Create transaction with custom fee estimator to avoid Pimlico-specific calls
    // The Safe4337Pack might be calling pimlico_getUserOperationGasPrice internally
    // We can provide a custom feeEstimator that uses standard ERC-4337 methods
    const safeOperation = await safe4337Pack.createTransaction({
      transactions: txList,
      options: {
        // Provide a custom fee estimator that uses standard ERC-4337 methods
        // This prevents the pack from calling Pimlico-specific methods
        feeEstimator: {
          preEstimateUserOperationGas: async (props) => {
            ...
            const userOpHex = convertUserOperationToHex(props.userOperation);
