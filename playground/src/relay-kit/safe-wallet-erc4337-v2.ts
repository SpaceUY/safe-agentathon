...
function buildUserOperationPayloadToEstimate(sourceUserOp: any, entryPoint?: string) {
  ...
  if (isEntryPointV06) {
    if (sourceUserOp.initCode) {
      userOpPayload.initCode = sourceUserOp.initCode;
    }
    //Mandatory for some reason
    if (sourceUserOp.paymasterAndData) {
      userOpPayload.paymasterAndData = sourceUserOp.paymasterAndData;
    } 
  } else {
    if (sourceUserOp.factory) {
      userOpPayload.factory = sourceUserOp.factory;
    }
    if (sourceUserOp.factoryData) {
      userOpPayload.factoryData = sourceUserOp.factoryData;
    }
    if (sourceUserOp.paymaster !== undefined && sourceUserOp.paymaster !== "0x") {
        userOpPayload.paymaster = sourceUserOp.paymaster;
        userOpPayload.paymasterData = sourceUserOp.paymasterData;
        userOpPayload.paymasterVerificationGasLimit = sourceUserOp.paymasterVerificationGasLimit ?? "0";
        userOpPayload.paymasterPostOpGasLimit = sourceUserOp.paymasterPostOpGasLimit ?? "0";
    }
  }

  return userOpPayload;
}
...
function createFeeEstimatorForAlchemy({
  ...
  return {
    preEstimateUserOperationGas: async (props: any) => {
      ...
      const userOpPayload = buildUserOperationPayloadToEstimate(sourceUserOp, props.entryPoint);
...
export async function execute(transactions?: Array<{ to: string; value: string; data: string }>): Promise<string> {
  ...
  try {
    ...
    let feeEstimator = createFeeEstimatorForAlchemy({
      bundlerUrl: BUNDLER_URL,
      maxFeePerGas,
      maxPriorityFeePerGas,
    });
